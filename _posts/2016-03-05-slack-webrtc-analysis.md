...
★3より、tricleICEには対応しているが、MCU側はtrickleではなく先にICE候補を投げつけてきていることが分かる。これはMCUやSFUで一般的だ。なぜなら、ICE候補が既に分かっているから。なお、ここでのポイントは、★3に登場しているIPアドレスが全てプライベートIPだということだ。ポート番号として12003/12004があるが、これはjanusのコンフィグなどで指定されているのだと思う。
この時点で、普通のUDPホールパンチでは接続できないことが分かる。（自宅やオフィスからは到達できないアドレスなので）
...
現行のブラウザはすでにICE-TCPをサポートしている。そのため、TURNを利用しなくてもTCPによるメディア・データのパケットを送受信可能である。
